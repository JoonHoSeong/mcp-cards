...
source: ../raw/skills/jetson-build-source/SKILL.md
...
- **Skill ID:** `jetson-build-source`
- **Purpose:** Use when you need to rebuild the BSP overlay — DT, OOT modules, or kernel — from changes under bsp_sources/. Triggers: build bsp, rebuild dtb, rebuild kernel.
- **Canonical source:** [../raw/skills/jetson-build-source/SKILL.md](../raw/skills/jetson-build-source/SKILL.md)
...
[`../../context/bsp-customization-workflow.md`] (source reference not bundled)
...
[`jetson-promote-image`](../raw/skills/jetson-promote-image/SKILL.md) reads the
...
[`../../references/bsp-customization-bpmp-dtb.md`] (source reference not bundled).
...
active target — see
[`../../references/bsp-customization-kernel-dtb.md`] (source reference not bundled)
...
[`../../context/target-platform-contract.md`] (source reference not bundled).
...
[`references/build-modes.md`](../raw/skills/jetson-build-source/references/build-modes.md). Drive
...
[`references/composite-registration.md`](../raw/skills/jetson-build-source/references/composite-registration.md#path-resolution).

**Gate (symmetric).** `$COMPOSITE_DTS` drives both directions:
present → apply the two idempotent patches below; absent → run the
[cleanup pass](../raw/skills/jetson-build-source/references/composite-registration.md#cleanup-pass-on-composite-removal)
...
[no-direct-in-tree-DT-edits rule] (source reference not bundled).
...
1. **Per-dir Makefile** — append `dtbo-y += <name>.dtbo` after the
   ...
   snippet + rationale:
   [`references/composite-registration.md`](../raw/skills/jetson-build-source/references/composite-registration.md#makefile-patch-idempotent-position-sensitive).
2. **Carrier flash conf** — append `OVERLAY_DTB_FILE+=",<name>.dtbo"`
   ...
   [`references/composite-registration.md`](../raw/skills/jetson-build-source/references/composite-registration.md#flash-conf-patch-idempotent-with-first-touch-pristine-import).
...
[`references/manifest-schema.md`](../raw/skills/jetson-build-source/references/manifest-schema.md).
...
[`references/long-tail-gotchas.md`](../raw/skills/jetson-build-source/references/long-tail-gotchas.md)
...
- **Kernel-DT changes: composite-overlay-only, split ownership.**
  ...
  ([rule + rationale] (source reference not bundled)).
...
- **Avoid bare `$0` in shell snippets inside this SKILL.md.** When
  ...
  `awk -v ROW="$0"`. See
  [`references/composite-registration.md`](../raw/skills/jetson-build-source/references/composite-registration.md#why-sed-based-splicing-not-awk).
...
| `No rule to make target '$KOUT/scripts/Makefile.compiler'` | `KERNEL_HEADERS` set to `$KOUT` instead of `$KS/kernel/$KERNEL_SRC_DIR` | Use the DT-mode snippet in [`references/build-modes.md`](../raw/skills/jetson-build-source/references/build-modes.md) verbatim — srctree vs objtree must not collapse. |
...
| Composite `.dtbo` silently missing from the output set | Per-dir Makefile insertion landed after `$(old-dtbo)` merge-back line | See [`references/composite-registration.md`](../raw/skills/jetson-build-source/references/composite-registration.md#makefile-patch-idempotent-position-sensitive) — insert after the last *literal-named* `dtbo-y +=` entry. |
...
| `host package missing: <pkg>` | `flex`/`bison`/`libssl-dev` refuse; others warn. | `sudo apt install <pkg>` per [`references/upstream-recipe.md`](../raw/skills/jetson-build-source/references/upstream-recipe.md). |

## See also

- [`references/build-modes.md`](../raw/skills/jetson-build-source/references/build-modes.md) — per-mode `make` snippets + install consolidation.
- [`references/manifest-schema.md`](../raw/skills/jetson-build-source/references/manifest-schema.md) — `.build-manifest.yaml` + `.build-state.yaml` schemas.
- [`references/composite-registration.md`](../raw/skills/jetson-build-source/references/composite-registration.md) — the "Register composite custom overlay" step full snippets + rationale.
- [`references/long-tail-gotchas.md`](../raw/skills/jetson-build-source/references/long-tail-gotchas.md) — invariants, deploy patterns, performance hints.
- [`references/upstream-recipe.md`](../raw/skills/jetson-build-source/references/upstream-recipe.md) — verbatim NVIDIA recipe, divergences, spec status.
- [`../../context/bsp-customization-workflow.md`] (source reference not bundled) — workspace edit protocol.
- [`../../references/bsp-customization-kernel-dtb.md`] (source reference not bundled) — composite custom-overlay contract.
- [`../../references/bsp-customization-bpmp-dtb.md`] (source reference not bundled) — BPMP-DTB edit contract (routed around this skill).
- [`../jetson-init-source/SKILL.md`](../raw/skills/jetson-init-source/SKILL.md) — Setup; authors `source.toolchain`.
- [`../jetson-promote-image/SKILL.md`](../raw/skills/jetson-promote-image/SKILL.md) — Deploy promoter; reads this skill's manifest.
