...
source: ../raw/skills/jetson-customize-clocks/SKILL.md
...
- **Skill ID:** `jetson-customize-clocks`
- **Purpose:** Use to lock/cap Jetson CPU/GPU/EMC clocks, toggle EMC/CPU DVFS, or change cpufreq governors by editing BPMP DTB and nvpower.sh pre-flash. Do NOT use for live tuning or nvpmodel edits.
- **Canonical source:** [../raw/skills/jetson-customize-clocks/SKILL.md](../raw/skills/jetson-customize-clocks/SKILL.md)
...
invariant in
[`../../context/bsp-customization-workflow.md#workflow-invariants`] (source reference not bundled) —
...
[`../../context/bsp-customization-software-layers.md#per-board-conf-dispatch--update_flash_args_common`] (source reference not bundled)),
...
Inspect both layers of the runtime ceiling — see [`references/clock-control-model.md#effective-runtime-ceiling`](../raw/skills/jetson-customize-clocks/references/clock-control-model.md#effective-runtime-ceiling) — before deciding on a `max-rate-custom` value.

Inspection cookbook (BPMP-side decompile + grep; nvpmodel-side awk over the boot default mode) is in [`references/bpmp-dtb-clock-edits.md#inspection-cookbook`](../raw/skills/jetson-customize-clocks/references/bpmp-dtb-clock-edits.md#inspection-cookbook).

For the nvpmodel layer see [`/jetson-customize-nvpmodel`](../raw/skills/jetson-customize-nvpmodel/SKILL.md).
...
mapping live in [`references/bpmp-dtb-clock-edits.md`](../raw/skills/jetson-customize-clocks/references/bpmp-dtb-clock-edits.md).
...
[`references/emc-dvfs-disable.md`](../raw/skills/jetson-customize-clocks/references/emc-dvfs-disable.md).
...
[Off-skill edits recipe] (source reference not bundled)
...
Function locations (`set_cpufreq_governor`, `set_devfreq_governor`), common-edit recipes (pin to Fmax, static rate, min/max bounds), and the `nvidia-l4t-init` package-upgrade caveat live in [`references/nvpower-sh-edits.md`](../raw/skills/jetson-customize-clocks/references/nvpower-sh-edits.md).
...
5. Set the boot-default nvpmodel mode to MAXN via [`/jetson-customize-nvpmodel`](../raw/skills/jetson-customize-nvpmodel/SKILL.md) — the per-clock nvpmodel cap clamps below `max-rate-maxn` regardless of BPMP DTB content.

Deploy [`/jetson-promote-image`](../raw/skills/jetson-promote-image/SKILL.md) → [`/jetson-flash-image`](../raw/skills/jetson-flash-image/SKILL.md) picks up the new BPMP DTB (via the overlay tracker) and the edited `nvpower.sh` (via the same overlay tracker) on the next flash.
...
- [`../../references/bsp-customization-bpmp-dtb.md`] (source reference not bundled) — canonical BPMP-DTB customization protocol (pristine import, decompile, edit, recompile, sanity-check, commit). Operation 1 of this skill is a content-only consumer; the protocol owns the mechanics.
- [`references/clock-control-model.md`](../raw/skills/jetson-customize-clocks/references/clock-control-model.md) — layer stack, two-ceilings overview, effective-runtime-ceiling formula.
- [`references/bpmp-dtb-clock-edits.md`](../raw/skills/jetson-customize-clocks/references/bpmp-dtb-clock-edits.md) — two-ceilings semantics, DTS edit form, nvpmodel ↔ BPMP clock-node mapping, inspection cookbook.
- [`references/emc-dvfs-disable.md`](../raw/skills/jetson-customize-clocks/references/emc-dvfs-disable.md) — full SoC-conditional EMC DVFS disable procedure with DTS snippets, detection, re-enable.
- [`references/nvpower-sh-edits.md`](../raw/skills/jetson-customize-clocks/references/nvpower-sh-edits.md) — `nvpower.sh` function locations + common-edit recipes + package-upgrade caveat.
- [`/jetson-customize-nvpmodel`](../raw/skills/jetson-customize-nvpmodel/SKILL.md) — sibling skill: nvpmodel power modes. The active mode's per-clock cap clamps below the BPMP DTB cap.
