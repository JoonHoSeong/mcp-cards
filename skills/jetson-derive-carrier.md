...
source: ../raw/skills/jetson-derive-carrier/SKILL.md
...
- **Skill ID:** `jetson-derive-carrier`
- **Purpose:** Bootstrap a custom carrier board by forking carrier files and scaffolding a DT overlay from the reference devkit. Use after jetson-init-source; not for module-level or kernel-DTB changes.
- **Canonical source:** [../raw/skills/jetson-derive-carrier/SKILL.md](../raw/skills/jetson-derive-carrier/SKILL.md)
...
[`target-platform-contract.md`] (source reference not bundled).
...
[workflow] (source reference not bundled));
conf-chain discovery follows
[per-board conf dispatch] (source reference not bundled).
...
[catalogue] (source reference not bundled) (unknown →
...
the
[commit message preview gate] (source reference not bundled).
...
split from
[commit batching] (source reference not bundled).
...
- **"chip: unknown" / fallback to `tegra234`** — module ID is not in
  the [catalogue] (source reference not bundled); update
