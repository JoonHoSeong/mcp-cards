...
source: ../raw/skills/jetson-init-target/SKILL.md
...
- **Skill ID:** `jetson-init-target`
- **Purpose:** Author a new Jetson target-platform profile (reference_devkit + optional custom_carrier) and update the active pointer. Use to create a target; not for switching existing profiles.
- **Canonical source:** [../raw/skills/jetson-init-target/SKILL.md](../raw/skills/jetson-init-target/SKILL.md)
...
[`../../context/target-platform-contract.md`] (source reference not bundled). It uses
[`../../references/bsp-platforms-catalogue.md`] (source reference not bundled)
as the canonical product / flash-config catalogue and
[`../../references/platform_template.yaml`] (source reference not bundled)
as the field schema.
...
[`jetson-init-image`](../raw/skills/jetson-init-image/SKILL.md); `source:` →
[`jetson-init-source`](../raw/skills/jetson-init-source/SKILL.md); `documents:` →
[`jetson-link-docs`](../raw/skills/jetson-link-docs/SKILL.md).
...
Follow the shared
[`quick_start_prefill` contract] (source reference not bundled).
This skill consumes:
...
1. If `target-platform/` does not exist, create it and copy
   [`../../references/active_target_template.yaml`] (source reference not bundled)
...
Read [`../../references/bsp-platforms-catalogue.md`] (source reference not bundled).
...
step. **Example:** see
[`references/ui-samples.md`](../raw/skills/jetson-init-target/references/ui-samples.md) for the
...
See [`references/gotchas.md`](../raw/skills/jetson-init-target/references/gotchas.md) for the full
pitfalls list (catalogue/template separation, NA handling, conf
collision rules, etc.).

## References

- [`../../references/bsp-platforms-catalogue.md`] (source reference not bundled) — product / flash conf catalogue (the source of truth for the "Pick the reference product" and "Pick the flash config" steps).
- [`../../context/target-platform-contract.md`] (source reference not bundled) — the contract this skill writes to.
- [`../../references/platform_template.yaml`] (source reference not bundled) — schema starter for a target profile (the source of truth for the "Optional: add custom carrier details" step).
- [`../../references/active_target_template.yaml`] (source reference not bundled) — schema starter for the active-target pointer.
- [`../jetson-set-target/SKILL.md`](../raw/skills/jetson-set-target/SKILL.md) — sibling skill: switch the active pointer among already-authored profiles.
- [`../jetson-init-image/SKILL.md`](../raw/skills/jetson-init-image/SKILL.md) — sibling skill: extract BSP image and author the `bsp_image:` block.
- [`../jetson-init-source/SKILL.md`](../raw/skills/jetson-init-source/SKILL.md) — sibling skill: clone shared repos and (optionally) author the `source:` block.
- [`../jetson-link-docs/SKILL.md`](../raw/skills/jetson-link-docs/SKILL.md) — sibling skill: author the `documents:` block by registering pre-downloaded files.
