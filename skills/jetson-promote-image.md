...
source: ../raw/skills/jetson-promote-image/SKILL.md
...
- **Skill ID:** `jetson-promote-image`
- **Purpose:** Use to promote overlay files and built artifacts into the staged BSP image. Do NOT use to flash or build. Triggers: promote bsp image.
- **Canonical source:** [../raw/skills/jetson-promote-image/SKILL.md](../raw/skills/jetson-promote-image/SKILL.md)
...
| **Build manifest** | `<source.root_path>/.build-manifest.yaml` | Rebuilt kernel `Image`, in-tree `.ko`, OOT `.ko`, NVIDIA DTBs | Build [`jetson-build-source`](../raw/skills/jetson-build-source/SKILL.md) writes here |
...
See [`references/copy-pass-snippets.md`](../raw/skills/jetson-promote-image/references/copy-pass-snippets.md#validate-the-two-channels)
...
[`references/build-source-freshness-gate.md`](../raw/skills/jetson-promote-image/references/build-source-freshness-gate.md).
...
See [`references/copy-pass-snippets.md`](../raw/skills/jetson-promote-image/references/copy-pass-snippets.md#pre-promote-collision-check-overlay-only)
...
[`jetson-build-source` v0.2.0](../raw/skills/jetson-build-source/references/manifest-schema.md).

See [`references/copy-pass-snippets.md`](../raw/skills/jetson-promote-image/references/copy-pass-snippets.md#enumerate-sources)
...
See [`references/copy-pass-snippets.md`](../raw/skills/jetson-promote-image/references/copy-pass-snippets.md#diff-aware-copy-into-bsp_image)
...
See [`references/kernel-image-and-initramfs.md`](../raw/skills/jetson-promote-image/references/kernel-image-and-initramfs.md#mirror-semantics--kernel-image-lands-in-two-places)
...
See [`references/kernel-image-and-initramfs.md`](../raw/skills/jetson-promote-image/references/kernel-image-and-initramfs.md#refresh-tool--l4t_update_initrdsh)
...
- **Kernel `Image` mirror + initramfs refresh.** Gated on copy-pass
  ...
  [`references/kernel-image-and-initramfs.md`](../raw/skills/jetson-promote-image/references/kernel-image-and-initramfs.md)
...
| DUT boots with stale kernel / modules after promote, modules fail to load with `disagrees about version of symbol …`, or initramfs ships pre-customize modules even after the refresh ran | The mirror / refresh gate didn't fire (manual hand-edit under `<LFT_DST>` outside the skill), or `rootfs/boot/Image` drifted from `kernel/Image` so the chrooted refresh built against the stale kernel | Force the gate by `sudo touch <LFT_DST>/kernel/Image` + re-run promote, or run the two steps manually: `sudo cp -p <LFT_DST>/kernel/Image <LFT_DST>/rootfs/boot/Image && cd <LFT_DST> && sudo ./tools/l4t_update_initrd.sh`. Then re-flash. See [`references/kernel-image-and-initramfs.md`](../raw/skills/jetson-promote-image/references/kernel-image-and-initramfs.md). |
...
- [`references/kernel-image-and-initramfs.md`](../raw/skills/jetson-promote-image/references/kernel-image-and-initramfs.md) — full contract for the kernel `Image` mirror + `l4t_update_initrd.sh` refresh: shell snippets, failure modes, tool semantics, output filenames.
- [`../../context/target-platform-contract.md`] (source reference not bundled) — target-platform contract.
- [`../../context/bsp-customization-workflow.md`] (source reference not bundled) — workspace edit protocol (this skill is the promote leg of Deploy).
- [`../jetson-init-source/SKILL.md`](../raw/skills/jetson-init-source/SKILL.md) — Setup; materializes the overlay tracker this skill reads (channel A) and authors `source.toolchain`.
- [`../jetson-build-source/SKILL.md`](../raw/skills/jetson-build-source/SKILL.md) — Build builder; writes the `.build-manifest.yaml` this skill reads (channel B).
- [`../jetson-flash-image/SKILL.md`](../raw/skills/jetson-flash-image/SKILL.md) — next leg; flashes the just-promoted bsp_image to the DUT.
- [`../jetson-validate-image/SKILL.md`](../raw/skills/jetson-validate-image/SKILL.md) — final leg; static + on-target validation.
