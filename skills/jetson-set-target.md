...
source: ../raw/skills/jetson-set-target/SKILL.md
...
- **Skill ID:** `jetson-set-target`
- **Purpose:** Switch the active Jetson target-platform pointer to an existing profile YAML. Use before customize/build/flash to change target; not for authoring profiles — use jetson-init-target instead.
- **Canonical source:** [../raw/skills/jetson-set-target/SKILL.md](../raw/skills/jetson-set-target/SKILL.md)
...
[`../../context/target-platform-contract.md`] (source reference not bundled). It only
...
- [`../../references/active_target_template.yaml`] (source reference not bundled) — schema starter for the active-target pointer (used to recreate it if the file is missing).
- [`../jetson-init-target/SKILL.md`](../raw/skills/jetson-init-target/SKILL.md) — sibling skill: author a new profile YAML.
