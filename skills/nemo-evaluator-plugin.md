...
source: ../raw/skills/nemo-evaluator-plugin/SKILL.md
...
- **Skill ID:** `nemo-evaluator-plugin`
- **Purpose:** Use when working on the Evaluator plugin CLI, jobs, SDK-backed specs, metric types, or plugin-owned Evaluator skills.
- **Canonical source:** [../raw/skills/nemo-evaluator-plugin/SKILL.md](../raw/skills/nemo-evaluator-plugin/SKILL.md)
...
See the LLM-judge spec example at [assets/specs/llm_as_judge.json](../raw/skills/nemo-evaluator-plugin/assets/specs/llm_as_judge.json).

### Metric Bundle Payloads

The checked-in [spec examples](../raw/skills/nemo-evaluator-plugin/assets/specs) use bundled SDK metrics. The fields under `metrics[*].payload` are generated by `bundle_metric(metric, CloudpickleMetricBundlePackager())`.

To see the pattern for configuring a pre-defined SDK metric, for example `ExactMatchMetric`, and converting it into bundled metric JSON, inspect `build_metric_bundle_example()` in [generate_example_specs.py](../raw/skills/nemo-evaluator-plugin/scripts/generate_example_specs.py) and run:
...
Examples of various specs are provided in the [assets/specs](../raw/skills/nemo-evaluator-plugin/assets/specs) directory.

#### Evaluate using `exact-match` metric

See the spec example at [assets/specs/exact_match_metric.json](../raw/skills/nemo-evaluator-plugin/assets/specs/exact_match_metric.json).
...
Uses an LLM to score responses. See the spec example at [assets/specs/llm_as_judge.json](../raw/skills/nemo-evaluator-plugin/assets/specs/llm_as_judge.json).
...
See examples of using the plugin SDK interface in [plugin_sdk_examples.py](../raw/skills/nemo-evaluator-plugin/assets/examples/plugin_sdk_examples.py).
...
For LLM-judge setup notes, see [LLM Judge Notes](../raw/skills/nemo-evaluator-plugin/references/llm-judge.md).

For evaluator API key auth, see [Evaluator API Auth](../raw/skills/nemo-evaluator-plugin/references/api-auth.md).

For local and cluster troubleshooting, see [Evaluation Troubleshooting](../raw/skills/nemo-evaluator-plugin/references/troubleshooting.md).
