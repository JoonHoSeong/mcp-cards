...
source: ../raw/skills/nemo-rl-session-memory/SKILL.md
...
- **Skill ID:** `nemo-rl-session-memory`
- **Purpose:** Manage durable working-session memory for coding agents. Use when a user asks to preserve or recover agent context across disconnects, VS Code restarts, long-running work, handoffs, or any session where important state should be written periodically under the repo's session directory. Do NOT use for: simple questions, short tasks, one-off commands, linting, or code review.
- **Canonical source:** [../raw/skills/nemo-rl-session-memory/SKILL.md](../raw/skills/nemo-rl-session-memory/SKILL.md)
