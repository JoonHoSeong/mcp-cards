...
source: ../raw/skills/nemotron-asr-finetune/SKILL.md
...
- **Skill ID:** `nemotron-asr-finetune`
- **Purpose:** Orchestration skill for NVIDIA Nemotron Speech (Riva) / NeMo ASR domain and language adaptation. Given a goal like "improve/fine-tune ASR for my domain or language", it scopes the task, picks the cheapest sufficient path (word boosting → n-gram LM → fine-tuning), delegates each stage to the right sub-skill (data generation, training, evaluation, deployment), and answers cost/time/data questions along the way.
- **Canonical source:** [../raw/skills/nemotron-asr-finetune/SKILL.md](../raw/skills/nemotron-asr-finetune/SKILL.md)
...
Run the loop below; each stage names the sub-skill it invokes. Full detail in [`references/workflow.md`](../raw/skills/nemotron-asr-finetune/references/workflow.md).
...
**Stages 4–8 are the fine-tune path** (`data → NeMo train → NeMo eval → Riva deploy`). Cheaper rungs (boosting, custom vocab, n-gram LM) take a **shorter branch owned by a single sub-skill** — don't force them through the full loop. See the branch-by-rung table in [`references/workflow.md`](../raw/skills/nemotron-asr-finetune/references/workflow.md) (§3b).

Throughout, answer the **"along the way"** questions (data volume, synthetic vs real, hours to reach a WER target, cost, GPU choice) — see [`references/planning-answers.md`](../raw/skills/nemotron-asr-finetune/references/planning-answers.md).

## Sub-Skills This Skill Calls

Detailed registry, invocation, and handoff contracts in [`references/sub-skills.md`](../raw/skills/nemotron-asr-finetune/references/sub-skills.md).
...
The scoping in Stage 3 selects the lowest-cost rung that can meet the target. Summary; full docs-grounded ladder in [`references/path-selection.md`](../raw/skills/nemotron-asr-finetune/references/path-selection.md).
