...
source: ../raw/skills/tao-analyze-changenet-rca/SKILL.md
...
- **Skill ID:** `tao-analyze-changenet-rca`
- **Purpose:** Performs deep Root Cause Analysis (RCA) on NVIDIA TAO Visual ChangeNet classification experiments with
- **Canonical source:** [../raw/skills/tao-analyze-changenet-rca/SKILL.md](../raw/skills/tao-analyze-changenet-rca/SKILL.md)
...
Before writing `RCA_Report.md`, run `ls rca_images/` to inventory thumbnails, and follow the **mandatory Image Embedding Protocol**: every visual-evidence table row must carry inline thumbnail columns using `![caption] (source reference not bundled)` syntax — a report without per-row images is incomplete and the hook will reject it.
