...
source: ../raw/skills/tao-generate-referring-expressions/SKILL.md
...
- **Skill ID:** `tao-generate-referring-expressions`
- **Purpose:** Four-step image referring-expression pipeline: turns images plus KITTI bounding-box labels into region
- **Canonical source:** [../raw/skills/tao-generate-referring-expressions/SKILL.md](../raw/skills/tao-generate-referring-expressions/SKILL.md)
...
4. **API access**: Ask the user which VLM endpoint they want to use. Present these five options and act on the choice:
   ...
   4. **vLLM** (self-hosted, OpenAI-compatible). Confirm whether the server is already running:
      - **Running** — collect `base_url`, `model_name`, and (optionally) `api_key`; set `vlm.backend: "openai"`.
      - **Not running** — follow [references/vllm_server.md](../raw/skills/tao-generate-referring-expressions/references/vllm_server.md) to install and launch a vLLM server, then collect `base_url`, `model_name`, and (optionally) `api_key`; set `vlm.backend: "openai"`.
...
See [references/configuration.md](../raw/skills/tao-generate-referring-expressions/references/configuration.md) for the full YAML structure, all parameters, model/endpoint setup, and error patterns.
...
Key configuration fields (full reference in [references/configuration.md](../raw/skills/tao-generate-referring-expressions/references/configuration.md)):
