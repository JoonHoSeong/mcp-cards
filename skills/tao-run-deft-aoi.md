...
source: ../raw/skills/tao-run-deft-aoi/SKILL.md
...
- **Skill ID:** `tao-run-deft-aoi`
- **Purpose:** Run the full DEFT AOI improvement loop for NVIDIA TAO VisualChangeNet / ChangeNet PCB inspection models: baseline evaluate, RCA, Cosmos AnomalyGen / AMP synthetic defects, k-NN mining, retraining, and deployment gating until FAR / recall KPI targets are met. Use for prompts like "run the DEFT loop", "fine-tune until FAR below 0.1% at recall=100%", or "improve my AOI ChangeNet model with RCA and synthetic defects"; do not use for standalone TAO training, one-off inference, generic anomaly generation, or RCA-only analysis.
- **Canonical source:** [../raw/skills/tao-run-deft-aoi/SKILL.md](../raw/skills/tao-run-deft-aoi/SKILL.md)
