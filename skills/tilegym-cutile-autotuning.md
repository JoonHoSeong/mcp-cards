...
source: ../raw/skills/tilegym-cutile-autotuning/SKILL.md
...
- **Skill ID:** `tilegym-cutile-autotuning`
- **Purpose:** Use when adding, modifying, optimizing, or debugging CuTile autotuning code. Trigger signals: `exhaustive_search` / `replace_hints` / `hints_fn` / `cuda.tile.tune` in code, `autotune` in filenames, or correctness/performance issues in autotuned CuTile kernels. Covers: tune-once/cache/launch pattern, per-architecture configs (sm80–sm120), parameter space design (tile sizes, occupancy, num_ctas), and 7 common pitfalls with solutions.
- **Canonical source:** [../raw/skills/tilegym-cutile-autotuning/SKILL.md](../raw/skills/tilegym-cutile-autotuning/SKILL.md)
...
For complex kernels (matmul with tile sizes, FMHA, FP8 with num_ctas), read the full guide below + [`kernel-type-templates.md`](../raw/skills/tilegym-cutile-autotuning/references/kernel-type-templates.md).
...
**5-step summary**: Classify kernel → Design search space ([`parameter-space-design.md`](../raw/skills/tilegym-cutile-autotuning/references/parameter-space-design.md)) → Implement using template ([`kernel-type-templates.md`](../raw/skills/tilegym-cutile-autotuning/references/kernel-type-templates.md)) → Validate with A/B test → Check Pitfall Checklist.
...
See [references/api-reference.md](../raw/skills/tilegym-cutile-autotuning/references/api-reference.md) for the full
...
See [references/workflow.md](../raw/skills/tilegym-cutile-autotuning/references/workflow.md) for the end-to-end
...
See [references/pitfalls.md](../raw/skills/tilegym-cutile-autotuning/references/pitfalls.md) for the full list of
...
|----------|----------|---------|
| **API Reference** | [`api-reference.md`](../raw/skills/tilegym-cutile-autotuning/references/api-reference.md) | `exhaustive_search` signature, `TuningResult`, tune-once/cache/launch pattern, `replace_hints`, kernel hints, `search_space` design, `grid_fn` patterns |
| **Workflow** | [`workflow.md`](../raw/skills/tilegym-cutile-autotuning/references/workflow.md) | End-to-end workflow: adding autotune to a new kernel, multi-architecture configs, `torch.autograd.Function` integration, Triton→CuTile transfer, optimizing existing configs |
| **Pitfalls** | [`pitfalls.md`](../raw/skills/tilegym-cutile-autotuning/references/pitfalls.md) | Common pitfalls: in-place corruption, compilation timeout, cold-cache skew, NCU interference, `search_space` exhaustion, FP8 precision, `replace_hints` recompilation |
| **Parameter Design** | [`parameter-space-design.md`](../raw/skills/tilegym-cutile-autotuning/references/parameter-space-design.md) | Per-kernel-type parameter spaces, cross-arch patterns, grid_fn patterns, pruning rules |
| **Search Strategies** | [`search-strategies.md`](../raw/skills/tilegym-cutile-autotuning/references/search-strategies.md) | Exhaustive search, A/B test methodology, DISABLE_AUTOTUNE pattern |
| **Templates** | [`kernel-type-templates.md`](../raw/skills/tilegym-cutile-autotuning/references/kernel-type-templates.md) | Copy-paste autotune templates for 8 kernel types |
| **Hardware** | [`hardware-constraints.md`](../raw/skills/tilegym-cutile-autotuning/references/hardware-constraints.md) | Per-architecture constraints, tile size ranges, num_ctas rules, TMA requirements |
...
|-----------|--------|-----------------|------------|-------------------|
| [`assets/examples/01_rmsnorm_occupancy_only/`](../raw/skills/tilegym-cutile-autotuning/assets/examples/01_rmsnorm_occupancy_only) | RMSNorm (reduction) | Occupancy-only `[1,2,4,8]` | Low | Most common pattern — no tile tuning, just find best occupancy. Grid = `NUM_SM * cfg.occupancy`. Not in-place. |
| [`assets/examples/02_matmul_full_search/`](../raw/skills/tilegym-cutile-autotuning/assets/examples/02_matmul_full_search) | GEMM C=A@B | Full: `TILE_M/N/K` + `occupancy` + `num_ctas` (sm90+) | High | Compute-bound kernel with multiple tunable dimensions. `args_fn` passes tile sizes as `ct.Constant[int]`. `grid_fn` depends on `cfg`. ≤30 configs. |
| [`assets/examples/03_rope_inplace_splitbuffer/`](../raw/skills/tilegym-cutile-autotuning/assets/examples/03_rope_inplace_splitbuffer) | RoPE embedding (in-place) | Occupancy-only, with split-buffer | Medium | In-place kernel MUST use split-buffer during search to avoid corruption. Search writes to scratch; final `ct.launch` uses real in-place args. |
