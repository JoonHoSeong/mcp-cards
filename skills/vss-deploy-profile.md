...
source: ../raw/skills/vss-deploy-profile/SKILL.md
...
- **Skill ID:** `vss-deploy-profile`
- **Purpose:** Use to select, configure, deploy, verify, debug, or tear down a VSS profile (base, search, lvs, warehouse, edge). Not for standalone microservices — use the vss-deploy-* skill.
- **Canonical source:** [../raw/skills/vss-deploy-profile/SKILL.md](../raw/skills/vss-deploy-profile/SKILL.md)
...
|---|---|---|
| "deploy vss" / "deploy base" | `base` | [`references/base.md`](../raw/skills/vss-deploy-profile/references/base.md) |
| "deploy alerts" / "alert verification" / "real-time alerts" / "deploy for incident report" | `alerts` | [`references/alerts.md`](../raw/skills/vss-deploy-profile/references/alerts.md) |
| "deploy lvs" / "video summarization" | `lvs` | [`references/lvs-profile.md`](../raw/skills/vss-deploy-profile/references/lvs-profile.md) |
| "deploy search" / "video search" | `search` | [`references/search.md`](../raw/skills/vss-deploy-profile/references/search.md) |
| "deploy warehouse" / "warehouse blueprint" / "vss warehouse" | `warehouse` | [`references/warehouse.md`](../raw/skills/vss-deploy-profile/references/warehouse.md) |
| "debug warehouse" / "warehouse not working" / "warehouse FPS low" / "warehouse BEV out of sync" | `warehouse` (debug) | [`references/warehouse-debug.md`](../raw/skills/vss-deploy-profile/references/warehouse-debug.md) |

**Edge hardware routing** (DGX Spark, AGX/IGX Thor): see [`references/edge.md`](../raw/skills/vss-deploy-profile/references/edge.md). DGX Spark uses the Spark Nano 9B standalone local LLM on port `30081`; AGX/IGX Thor uses the Edge 4B standalone vLLM fallback.
...
1. **Repo path** — auto-detect `video-search-and-summarization/` before
   ...
   commands.
2. **Credential gates** — see [`references/credentials.md`](../raw/skills/vss-deploy-profile/references/credentials.md): `NGC_CLI_API_KEY` for local/local_shared NIM pulls, `NVIDIA_API_KEY` for remote NIM endpoints, and `HF_TOKEN` for edge recipes that use gated HF models.
3. **System prerequisites (GPU driver, Docker, NVIDIA Container Toolkit, kernel sysctls, and — if `ufw` is active — the [Docker-bridge→host firewall allow](../raw/skills/vss-deploy-profile/references/prerequisites.md#firewall) so bridge NIMs can fetch clips from host-mode VST)** — full checks in [`references/prerequisites.md`](../raw/skills/vss-deploy-profile/references/prerequisites.md). Canonical hardware/driver matrix is the [VSS prerequisites page](https://docs.nvidia.com/vss/3.2.0/prerequisites.html).
...
lives in [`references/prerequisites.md`](../raw/skills/vss-deploy-profile/references/prerequisites.md#repo-detect).
...
in [`references/prerequisites.md`](../raw/skills/vss-deploy-profile/references/prerequisites.md#preflight).
For DGX Spark / IGX Thor / AGX Thor, also run the cache-cleaner check in
[`references/edge.md`](../raw/skills/vss-deploy-profile/references/edge.md#cache-cleaner-every-edge-deploy).
...
[`references/prerequisites.md`](../raw/skills/vss-deploy-profile/references/prerequisites.md#preflight)
...
> **Edge shared mode is platform-specific.** Full recipes are in [`references/edge.md`](../raw/skills/vss-deploy-profile/references/edge.md).
...
Full procedure lives in [`references/teardown.md`](../raw/skills/vss-deploy-profile/references/teardown.md).
...
[`references/credentials.md`](../raw/skills/vss-deploy-profile/references/credentials.md), including
...
| **`HOST_IP`** | In-cluster dial address: `ip route get 1.1.1.1` src (like `dev-profile.sh`; correct on LAN + cloud). If that interface is a VPN/tunnel, fall back to the LAN IP and **prompt the user** — [Network addressing](../raw/skills/vss-deploy-profile/references/prerequisites.md#addressing). |
| **`EXTERNAL_IP`** | Browser-facing address; defaults to `${HOST_IP}`. Override when the browser path differs — cloud public IP, Brev secure-link (Step 1d), or tunnel; **ask the user where they browse from if unsure**. [Network addressing](../raw/skills/vss-deploy-profile/references/prerequisites.md#addressing). |
...
Layout (asset paths, ownership, mount points, profile-specific subdirs) is documented in [`references/data-directory.md`](../raw/skills/vss-deploy-profile/references/data-directory.md). Read that file before deploying for the first time on a host or when changing profiles.
...
- **not Brev** → skip the rest of this step and **do not read [`references/brev.md`](../raw/skills/vss-deploy-profile/references/brev.md)**; keep the normal `${HOST_IP}`-based `EXTERNAL_IP`.
- **on Brev** → apply the Brev secure-link overrides from [`references/brev.md` § Setup flow](../raw/skills/vss-deploy-profile/references/brev.md#setup-flow) to `generated.env` (NOT `.env`). Those set `EXTERNAL_IP` / `VSS_PUBLIC_HOST` to the secure-link domain **and** `VSS_PUBLIC_HTTP_PROTOCOL=https` / `VSS_PUBLIC_WS_PROTOCOL=wss` / `VSS_PUBLIC_PORT=443` — setting `EXTERNAL_IP` alone leaves `http://…:7777` UI/API/WS links that the browser blocks as mixed content.
...
in [`references/env-overrides.md`](../raw/skills/vss-deploy-profile/references/env-overrides.md). Each profile
...
Unexpanded `${VAR}` tokens in `resolved.yml` mean compose did not see those env values. Diagnostic procedure and common culprits live in [`references/troubleshooting.md`](../raw/skills/vss-deploy-profile/references/troubleshooting.md).
...
**Gate 0 — container count must be > 0.** Refuse to proceed past `up -d` until the started count (`docker compose -f resolved.yml ps -q | wc -l`) is non-zero and ≥ the expected count (`config --services | wc -l`); a zero/short count almost always means a missing `--env-file` in Step 5. The exact gate plus the full readiness procedure live in [`references/readiness.md`](../raw/skills/vss-deploy-profile/references/readiness.md).
...
switch — follow [`references/teardown.md`](../raw/skills/vss-deploy-profile/references/teardown.md). Always tear down
...
[`references/troubleshooting.md`](../raw/skills/vss-deploy-profile/references/troubleshooting.md#nim-probes).
...
[`references/troubleshooting.md`](../raw/skills/vss-deploy-profile/references/troubleshooting.md) — start there
